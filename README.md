...
| <a name="input_policy_set_definition"></a> [policy\_set\_definition](#input\_policy\_set\_definition) | The policy set definition to assign. | <pre>object({<br>    id                          = string<br>    policy_definition_reference = list(object({ policy_definition_id = string }))<br>    display_name                = optional(string)<br>    description                 = optional(string)<br>  })</pre> | n/a | yes |
